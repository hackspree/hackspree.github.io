...
- Each social/profile icon shows a custom hover hint with link-specific copy below the icon after a short intentional hover delay.
...
- Social/profile hover hints should be implemented in CSS using per-link metadata on the anchor so the delay, below-icon placement, and styling are consistent across icons.
